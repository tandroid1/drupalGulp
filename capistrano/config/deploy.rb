...
task :set_paths do
  ...
  task :setup, :except => { :no_release => true } do
    ...
    sudo("mkdir -p #{deploy_to} #{releases_path} #{shared_path} #{shared_default_site_path} #{shared_default_site_files_path} #{shared_default_site_private_path} #{shared_default_site_tmp_path}")
    sudo("chown #{file_owner_user} #{deploy_to} #{releases_path} #{shared_path} #{shared_default_site_path} #{shared_default_site_files_path} #{shared_default_site_private_path} #{shared_default_site_tmp_path}") 
  ...
  task :upload_local_files do
    transaction do  
      ...
      if (files_to_upload == "yes")
        ...
        sudo("chown -R #{file_owner_user}:#{web_server_user} -R #{default_files}")
        sudo("sudo chmod 775 -R #{default_files}") 
  ...
  task :create_site do
      ...
      sudo("#{mysql_run} -e \"#{mysql_create}\"")

      sudo("#{mysql_run} -e \"#{mysql_grant}\"")
      ...
      if ("#{mysql_remote_empty}" == "0")
        sudo("gunzip < #{db_dump_remote_full} | #{mysql_run} #{mysql_db_name_remote}")
      else
        db_already_populated = true
      end

      sudo("rm #{db_dump_remote_full}")
      ...
      if ("#{apache_conf_dir_exists}" == "yes")
        ...
        if ("#{plesk_server}" == "yes")
          ...
          if ("#{apache_skel_exists}" == "yes")
            ...
            sudo("cp #{apache_skel} #{apache_vhost}")
            sudo("sed -i 's/hostnamefqdn/#{remote_hostname}/g' #{apache_vhost}")
            sudo("sed -i 's/e3skeldomain/#{site_url}/g' #{apache_vhost}")
            sudo("sed -i 's/sitehtdocs/#{site_htdocs}/g' #{apache_vhost}")
            sudo("sed -i 's:e3skelsiteroot:#{site_root}:g' #{apache_vhost}")
